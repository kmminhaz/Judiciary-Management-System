...
const CriminalCase = () => {
  ...
  useEffect(() => {
    fetch(
      "http://localhost/projects/Defence/judiciary-system-management/src/API/criminalCases.php"
    )
      .then((res) => res.json())
  ...
  const searchFor = (event) => {
    ...
    fetch(
      `http://localhost/projects/Defence/judiciary-system-management/src/API/search.php?target=criminal&search=${text.toLowerCase()}`
    )
      .then((res) => res.json())
      .then((data) => setData(data));
  };
          ...
          {/* <section>
            <form action="../../../API/search.php" method="POST">
              ...
              <div className='d-flex justify-content-center'>
                <button type='submit' name="submit" className='btn btn-primary btn-lg mt-5'>
                  Filter Search
                </button>
              </div>
            </form>
